...
config = LoraConfig(
    r=1, lora_alpha=16, target_modules=target_modules, lora_dropout=0.1, bias="none", task_type="CAUSAL_LM"
)
...
# ### Training
data = load_dataset('stoddur/rmh_tokenized_512') #load_from_disk('datasets/rmh_tokenized_2048') #load_dataset("imdb")

#out = data_collator([data["train"][i] for i in range(5)])
#for key in out:
